...
if __name__ == "__main__":
    ...
    for mesh_dir, mesh_list in zip(mesh_dir_list, mesh_lists):
        for mesh_filename in mesh_list:
            ...
            mesh = obj_mesh_loader.load(adversarial_meshes[6])
            ...
            stable_poses, _ = mesh.compute_stable_poses(threshold=0.001)
            ...
            for i in tqdm(range(num_samples_per_obj)):
                sp1_idx, sp2_idx = np.random.randint(0,len(stable_poses), 2)
                # sp1_idx, sp2_idx = sp1_idx,sp1_idx
                # print("Iteration", i, "Stable Poses are", sp1_idx, "and", sp2_idx)
                sp1, sp2 = stable_poses[sp1_idx].copy(), stable_poses[sp2_idx].copy()
                sp1[:2,3] += np.random.uniform(-0.02,0.02,2)
                sp1[2,3] += 0.2
                sp2[:2,3] += np.random.uniform(-0.02,0.02,2)
                sp2[2,3] += 0.2

                ctr_of_mass1 = sp1[0:3, 3]
                ctr_of_mass2 = sp2[0:3, 3]
                ...
                rand_transform2 = Generate_Random_Z_Transform(ctr_of_mass2) @ sp2

                scene.set_pose(object_node, pose=rand_transform)
                image1 = renderer.render(scene, flags=RenderFlags.DEPTH_ONLY)
                scene.set_pose(object_node, pose=rand_transform2)
                ...
                image2 = Zero_BG(image2, DR=False)
                pc1 = torch.Tensor(demean(pointcloud(image1))).float().to(device)
                pc2 = torch.Tensor(demean(pointcloud(image2))).float().to(device)
                if config['debug']:
                    Plot_Image(image1, "stp" + str(sp1_idx) + ".png")
                    # Plot_PC([pc1.cpu().numpy().T], "plots/stp" + str(sp1_idx)+ "_pc.png")
                    # print(cur_pc.cpu().numpy().T)

                result = is_match(pc1, pc2)
                # print("Predicting to be", result[0])
                # if not result[0]:
                #     sys.exit()
                results.append([sp1_idx,sp2_idx,sp1_idx==sp2_idx, result[0], result[1]])
    ...
    print("Added datapoints to dataset from ", len(objects_added), "objects")

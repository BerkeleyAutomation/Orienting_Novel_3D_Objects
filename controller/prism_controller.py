...
def Matrix_Percent_Fit(object_id, obj_matrix, prism_matrix):
    obj_id = 0
...
if __name__ == "__main__":
    ...
    for mesh_dir, mesh_filename in Load_Mesh_Path():
        ...
        prism_mesh = Mesh.from_trimesh(Aligned_Prism_Mesh(mesh,expand=1.1))
        ...
        for j in tqdm(range(num_runs_per_obj)):
            matrices, quats = {}, {}
            goal_pose_matrix = Get_Initial_Pose(0.005,0.18,0.19)
            ...
            for i in range(total_iters+1):
                ...
                fit = Matrix_Percent_Fit(obj_id, m, matrices["goal"])

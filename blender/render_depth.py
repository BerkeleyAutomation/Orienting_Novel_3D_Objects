...
if "__main__" == __name__:
    ...
    num_frames = 10
    # Input mesh
    mesh_file = 'nozzle_2731189.obj'
    # Output prefix
    quaternion_output_file = os.path.join(os.getcwd(), "quaternion_nozzle.txt")
    depth_output_prefix = os.path.join(os.getcwd(), "depth_nozzle_")
    # ======================================== End of Options
    ...
    table.location = (0, 0, 0.0)
    #table.active_material = create_shadeless_material(rgba=(0,0,0,1))
    meshes = ['CatLying_800_tex.stl', 'high_heel_3dnet.stl',
              'piggy_2926476.stl', 'pipe_fork_5049153.stl',
              'tennis_shoe_3dnet.stl', 'two_hand_wrench_3133822.stl',
              'mechanical_part_262891.stl', 'nerf_gun_base_3994236.stl',
              'nozzle_5222583.stl', 'hex_vase_3276254.stl']
    for mesh_file in meshes:
        out_mesh_name = mesh_file[:-4]

        #import_mesh('nozzle_5222583.stl')
        mesh = import_mesh(mesh_file)
        # Output prefix
        cwd_path = os.path.join(os.getcwd(), 'shivin_exp')
        quaternion_output_file = os.path.join(cwd_path, out_mesh_name+'.txt')
        depth_output_prefix = os.path.join(cwd_path, out_mesh_name+"_")
        mesh.location = (0.0, 0.0, 0.03)
        print(mesh.location)
